...
2. `inputs`：这是一个数组，描述了函数的输入参数。每个参数都是一个对象，包含`name`（参数名）、`type`（参数类型）和`indexed`（是否是索引参数，只对事件有效）三个字段。
3. `name`：这是函数或事件的名称。
4. `outputs`：这是一个数组，描述了函数的返回值。每个返回值都是一个对象，包含`name`（返回值名）和`type`（返回值类型）两个字段。
5. `payable`：这是一个布尔值，表示函数是否可以接收以太币。
6. `stateMutability`：这是一个字符串，表示函数的状态可变性。它的值可以是`pure`（函数不读取也不修改状态）、`view`（函数不修改状态，但可能读取状态）、`nonpayable`（函数可能修改状态，但不能接收以太币）或`payable`（函数可能修改状态，也可以接收以太币）。
7. `type`：这是一个字符串，表示元素的类型。它的值可以是`function`、`constructor`、`fallback`（回退函数）或`event`。
以上就是ABI的主要字段，但请注意，不同的函数或事件可能并不包含所有的字段。例如，构造函数（constructor）就没有`name`和`outputs`字段，因为它在部署合约时被调用，没有名称，也没有返回值。